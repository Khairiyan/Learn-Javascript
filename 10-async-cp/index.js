...
async function getDataPeopleById(id) {
  // TODO: answer here
  stringResult = await getStarWarsData("https://swapi.dev/api/people/" + id);
  let output = JSON.parse(stringResult);
  return `${output.name}, memiliki tinggi ${output.height}cm dan lahir pada tahun ${output.birth_year}`;
}
