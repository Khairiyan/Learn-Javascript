...
function getDataPeopleByIdWithFilms(peopleId) {
  ...
  const result = promiseStarWarsData(url)
    .then((data) => {
      const film = [];
      data.films.forEach((urlFilms) => {
        film.push(promiseStarWarsData(urlFilms));
      });

      return Promise.all(film)
        .then((dataFilm) => {
          const filmObject = [];
          dataFilm.forEach((dataFilm) => {
            filmObject.push({
              title: dataFilm.title,
              episode_id: dataFilm.episode_id,
            });
          });
          data.films = filmObject;
